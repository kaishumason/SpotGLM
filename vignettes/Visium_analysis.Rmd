...
Here $L_s$ is the library size of spot $s$ and $\pi_{s,t}$ is the deconvolution estimate for cell type $t$ in spot $s$. This model can be fit using a spotGLM 
...
for(j in c(1:num_genes)){
  ...
  res[[j]] = spotglm::run_model(y = data$counts[,j],X = data$niche, lambda = data$deconv,
                             ...
                             initialization = "intercept",intercept_col = 1,min_reads_per_1000 = 0.05,batch_size = 500)
}
...
majority_subclone <- colnames(data$deconv)[max.col(data$deconv, ties.method = "first")]

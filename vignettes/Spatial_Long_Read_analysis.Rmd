---
title: "Spatial_Long_Read_analysis"
...
For a gene $g$ let the isoform relative proportion the isoform of interest in spot $s$  be $\gamma_s^g$. $\gamma_s^g$ is a weighted average of the isoform relative proportions of the cell types that make up $s$ with cell type $t$'s weight equal to the cell type mean expression of gene $g$, $\mu_{g,t}$ times the proportion of cell type $t$ in spot $s$, $\pi_{s,t}$. Let $\gamma_t^g$ be the isoform relative proportion for cell type $t$. Then 
...
$$logit(\gamma^g_i) = X_i\beta^{CT(i),g}$$
...
We first fit the intercept model as described above.We make use of the spotglm function "run_model". This function functions like the normal glm function. We specify our response $y$ which we take to be the gene expression values for a gene $g$. our covariates $X$ is just the intercept matrix. $\lambda$ is our deconvolution which we make sure has rowSums that sum to 1. Our offset is the library size of the each spot. The initialization used is "full" since we want to intialize all prameters. Our family is "spot poisson" since we are fitting a poisson model. 
```{r}
#get number of genes 
...
for(j in c(1:ngenes)){
  intercept_estimate[[j]]=spotglm::run_model(y = data$total_gene_expression[,j],
                               ...
                               initialization = "full",
                               family = "spot poisson",intercept_col = 1,
                               min_reads_per_1000 = 0)
...
We next fit the binomial spotGLM as described above. We again use the "run model" function. This time however, the family is "spot binomial" since we are running a binomial model. The weights are the total gene exression for the gene $g$ and we weight our deconvolution matrix $\lambda$ by $\beta_{0,g}^t$ which we fit from the previous step. 
```{r}
...
We now test for cell type specific isoform switching across regions. We append the matrices outputted from the "compute contrast significance" function and perform a global BH correction. We then print out the genes that show significant isoform switching. 
```{r}
...
Here we show a gene, Ap3s1, that shows evidence of isoform switching betwene the granule layer and other layers in Neurons. Because Neurons are the only important cell types for this gene, we can safely just plot overall isoform relative proportion and attribute any differences to Neurons. 
```{r,fig.width=6, fig.height=6}
gene <- "Ap3s1"
